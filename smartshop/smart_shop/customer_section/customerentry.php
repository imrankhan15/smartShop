	...
	$errors=array();
	 ...
	 if(empty($errors)){
		...
		if(!$connection){
		 ...
		 $query = "INSERT INTO customer (name,mobile,email,address,userId) VALUES (
					'{$name}','{$mobile}','{$email}','{$address}',{$user_id}
					)";
...
<?php include("../includes/header.php"); ?>
	<div class="container">
		...
		<p>The new customer for your Shop...........</p>
		</div>
	</div>
	 <div class="container">
		<?php if (!empty($message)) {echo "<p>" . $message . "</p>";} ?>
			<?php if (!empty($errors)) { display_errors($errors); } ?>
      <form class="form-signin" method="post" action="customerentry.php">
      
		
		</br>
        <input name="name" class="form-control" placeholder="Customer Name" required="" autofocus="">
		</br>
