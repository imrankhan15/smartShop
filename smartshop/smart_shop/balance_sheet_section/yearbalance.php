...
<?php include("../includes/header.php"); ?>
	<div class="container">
		<div class="jumbotron">
		<h1>Show Last Years Balance</h1>
		
		<table>
		<caption>Sales Balance</caption>
		<?php
		$last_year_today = mktime(0,0,0,date("m"),date("d")+1,date("Y")-1);
		//echo "last_year_today is ".date("Y-m-d", $last_year_today);

		$last_year_today =date("Y-m-d", $last_year_today);
		 $date=date("Y-m-d");
		  ...
		  $query .= "AND C.date BETWEEN '{$last_year_today}' AND '{$date}' ";
				...
				<caption>Buy Balance</caption>
				 ...
				 $date=date("Y-m-d");
					...
					die("Database query failed: ".mysql_error());
					    ...
					    if( mysql_num_rows($result_set)>0)
				{
					   echo "<tr><td>"."STORE NAME"."</td><td>"."ITEM "."</td><td>"."DATE"."</td><td>"."SUPPLIER "."</td><td>"."QUANTITY"."</td><td>"."PRICE"."</td><td>"."PAID BY"."</td></tr>";
		...
		?>
				...
				<caption>Other Cost</caption>
				 ...
				 $query= "SELECT A.description,A.date,A.paid_by,A.cost,B.name ";
					  ...
					  $query .= "WHERE A.userId = {$user_id} ";
					  $query .= "AND A.date BETWEEN '{$last_year_today}' AND '{$date}' ";
		...
		<table>
				...
				<table>
				  ...
				  $query .= "AND B.date BETWEEN '{$last_year_today}' AND '{$date}' ";
