...
<?php include("../includes/header.php"); ?>
	<div class="container">
		<div class="jumbotron">
		<h1>Show Last Weeks Balance</h1>
		
		<table>
		<caption>Sales Balance</caption>
		<?php
		$last_week_today = mktime(0,0,0,date("m"),date("d")-7,date("Y"));
		//echo "last_week_today is ".date("Y-m-d", $last_week_today);

		$last_week_today =date("Y-m-d", $last_week_today);
		 $date=date("Y-m-d");
		 ...
		 $query= "SELECT A.sell_id,A.item_id,A.quantity,A.price,A.sold_by,B.name,C.date ";
		  ...
		  $query .= "AND C.date BETWEEN '{$last_week_today}' AND '{$date}' ";
		...
		?>
				...
				<caption>Other Cost</caption>
				 ...
				 $query= "SELECT A.description,A.date,A.paid_by,A.cost,B.name ";
					  ...
					  $query .= "WHERE A.userId = {$user_id} ";
					  $query .= "AND A.date BETWEEN '{$last_week_today}' AND '{$date}' ";
		...
		<table>
				...
				<table>
				 <caption>Owner Assets</caption>
				  ...
				  $query .= "AND B.date BETWEEN '{$last_week_today}' AND '{$date}' ";
